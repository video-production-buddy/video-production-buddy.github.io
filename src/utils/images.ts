...
export const adaptOpenGraphImages = async (
  ...
  const adaptedImages = await Promise.all(
    openGraph.images.map(async (image) => {
      ...
      // Public assets are copied unchanged, so their configured dimensions
      // must not be replaced by the optimizer's default 1200×626 dimensions.
      if (typeof resolved === 'string' && resolved.startsWith('/')) {
        return { ...image, url: String(new URL(resolved, astroSite)) };
      }
